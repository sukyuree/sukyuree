...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_token) {
    ...
    directory_t *created_or_not_dir;
    file_t *created_or_not_file;
    for(int i=0;i<num_token-1;i++){
        bool is_create = false;
        created_or_not_dir=find_create_dir(current_dir, token_list[i], &is_create);
        if(is_create==true){
            current_dir->dir_list[current_dir->num_dir]=created_or_not_dir;
            current_dir->num_dir++;
            current_dir=created_or_not_dir;
        }
        else current_dir=created_or_not_dir;
        if(i==num_token-2){
            is_create = false;
            created_or_not_file=find_create_file(current_dir, token_list[num_token-1], &is_create);
            if(is_create==true){
                current_dir->file_list[current_dir->num_file]=created_or_not_file;
